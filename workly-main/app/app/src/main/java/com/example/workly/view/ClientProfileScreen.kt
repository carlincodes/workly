...
fun ClientProfileScreen(
    ...
    uiState: ProfileUiState, // Estado centralizado vindo de fora
    onProfileDataChanged: (ProfileData) -> Unit, // Evento disparado quando qualquer campo muda
    onSaveClicked: () -> Unit, // Evento de clique em salvar
    onLogoutClicked: () -> Unit // Evento de clique em sair da conta
) {
    Scaffold(
        topBar = {
            TopAppBar(
        ...
        if (uiState.isLoading) {
            ...
            Column(
                ...
                OutlinedTextField(
                    value = uiState.profileData.phone,
                    onValueChange = { onProfileDataChanged(uiState.profileData.copy(phone = it)) },
                    label = { Text("Telefone") },
                    modifier = Modifier.fillMaxWidth().padding(bottom = 8.dp),
                    singleLine = true
                )
